...
namespace HelperWebSL
{
    public class serenityHelper
    {
        ...
        public bool _Login(string Username, string Password, string CompanyDB,string dbname )
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    var Request = baseurl+ "/Account/Login";
        ...
        public JObject GetARInvoicesJobj()
        {
            var url = baseurl + "/Services/ARInvoice/Document/GetAll";
            var response = httpclient.GetAsync(url).Result;
            var result = response.Content.ReadAsStringAsync().Result;
        ...
        internal bool CreateIncomingPayment(IncomingPaymentPosting incomingPayment)
        {
            ...
            var datum = new StringContent(JsonConvert.SerializeObject(incomingPayment, Newtonsoft.Json.Formatting.Indented), Encoding.UTF8, "application/json");
            var response = httpclient.PostAsync(url, datum).Result;
            var result = response.Content.ReadAsStringAsync().Result; 
            return response.IsSuccessStatusCode;
        }

        internal List<(string BankCode, string BankName)> GetBanks()
        {
            var url = baseurl + "/Services/IncomingPayment/Payment/GetBanks";
            var response = httpclient.GetAsync(url).Result;
            var result = response.Content.ReadAsStringAsync().Result;
            JArray arr = JArray.Parse(result);
            var banks = arr.Select(x => (x["BankCode"].ToString(), x["BankName"].ToString())).ToList();
            return banks;
        }
    }
}
