namespace SAPWebPortal.APInvoiceLine {
    ...
    export class DocumentLineEditor extends _Ext.GridEditorBase<SAPWebPortal.APInvoiceLine.DocumentLineRow> {
        protected getColumnsKey() { return "APInvoiceLine.DocumentLine"; }
        protected getDialogType() { return SAPWebPortal.APInvoiceLine.DocumentLineDialog; }
        protected getLocalTextPrefix() { return SAPWebPortal.APInvoiceLine.DocumentLineRow.localTextPrefix; }
        ...
        deleteEntity(id) {
            ...
            this.DocTotalHelper = new SAPWebPortal.Helpers.DocTotalHelper(SAPWebPortal.APInvoice.DocumentDialog.Form, "");
            return ans;
        }
        protected getButtons() {
            ...
            buttons.push({
                ...
                onClick: () => {
                    ...
                    var HeaderForm = SAPWebPortal.APInvoice.DocumentDialog.Form;
                    ...
                    if (HeaderForm.CardCode != null && HeaderForm.CardCode.value != undefined && HeaderForm.CardCode.value != "") {
                        if (HeaderForm.DocType.value == "dDocument_Items") {
                            ...
                            var orderDetails = selectedItems2.map<SAPWebPortal.APInvoiceLine.DocumentLineRow>(r => {
                            var itemname = "";
                            var data = { Code: r.ItemCode, DBName: localStorage.getItem("DBName") };
                            Q.serviceCall({
                                url: Q.resolveUrl('~/Services/APInvoice/Document/getItemNameFromItemCode'),
                                request: data,
                                async: false,
                                onSuccess: response => {
                            ...
                            var cardcode = SAPWebPortal.APInvoice.DocumentDialog.Form.CardCode.value;

                            var data = {
                                ItemCode: r.ItemCode, CardCode: cardcode
                            }
                            Q.serviceCall({
                                url: Q.resolveUrl('~/Services/APInvoiceLine/DocumentLine/getItemDetail'),
                                request: data,
                                async: false,
                                onSuccess: response => {
                                    v.Quantity = 1
