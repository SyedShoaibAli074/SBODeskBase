...
namespace SAPWebPortal.DeliveryLine.Endpoints
{
    [Route("Services/DeliveryLine/DocumentLine/[action]")]
    ...
    public class DocumentLineController : ServiceEndpoint
    {
        ...
        {
            var _cardcode = ItemCode.Code;
            var DBName = ItemCode.DBName;
            double Price = 0; string WhsCode = ""; string UOM = ""; string TaxCode = ""; double Discount = 0; 
            double UnitsOfMeasurment = 0; double LineTotal = 0.0; string ObjCode = ""; string ItmsGrpCod = "";
            try
            {
                var query = String.Format(DBHelper.GetQuery("ItemPrice", DBName), _cardcode);
            ...
            var res = new { Price, WhsCode, UOM, TaxCode,Discount, UnitsOfMeasurment,LineTotal };
            return Json(res);
        }
    }
}