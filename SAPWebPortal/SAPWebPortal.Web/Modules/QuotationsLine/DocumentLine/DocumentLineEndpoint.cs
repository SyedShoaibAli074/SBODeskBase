...
namespace SAPWebPortal.QuotationsLine.Endpoints
{
    [Route("Services/QuotationsLine/DocumentLine/[action]")]
    ...
    public class DocumentLineController : ServiceEndpoint
    {
        ...
        public JsonResult getItemDetail(GetItemDetailData data)
        {
            ...
            var res = new { Price, WhsCode, UOM, TaxCode, Discount, UnitsOfMeasurment, LineTotal, GrossTotal };
            return Json(res);
        }
        public JsonResult getSapId(string UserName)
        {
            var UserId = "";
            try
            {
                var query = String.Format(DBHelper.GetQuery("Query_57"), UserName);
