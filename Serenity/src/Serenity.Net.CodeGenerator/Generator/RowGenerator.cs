    ...
    public class RowGenerator
    {
        ...
        private static EntityField ToEntityField(FieldInfo fieldInfo, int prefixLength)
        {
            ...
            if (fieldInfo.IsIdentity)
                ...
                flags = null;
            string dataType = "";
            var fieldType = fieldInfo.DataType;// SchemaHelper.SqlTypeNameToFieldType(fieldInfo.DataType, fieldInfo.Size, out string dataType);
            dataType  = fieldType;
            ...
            string module, string connectionKey, string entityClass, string permission, GeneratorConfig config, bool net5Plus)
        {
            ...
            model.Fields = new System.Collections.Generic.List<EntityField>();
            model.Joins = new System.Collections.Generic.List<EntityJoin>();
            model.Instance = true;
            ...
            if (fields.Count() == 0)
            {
                var name = "SAPB1." + model.ClassName;
                Console.WriteLine(name);
                var type = Type.GetType(name);
                var properties = type.GetProperties();
                foreach (var property in properties)
                {
                    var type1 = typeof(SAPB1.BusinessPartnerGroup).GetProperties().First().PropertyType;
                    fields.Add(new FieldInfo() {  DataType = property.PropertyType.ToString(), FieldName = property.Name });
                }
            }
            ...
            System.Collections.Generic.List<string> baseRowFieldList = new();
            foreach (var k in config.BaseRowClasses ?? new System.Collections.Generic.List<GeneratorConfig.BaseRowClass>())
            {
                var b = k.ClassName;
                var f = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                var fl = new System.Collections.Generic.List<string>();
                bool skip = false;
                foreach (var s in k.Fields ?? new System.Collections.Generic.List<string>())
                {
            ...
            foreach (var s in config.RemoveForeignFields ?? new System.Collections.Generic.List<string>())
            {
                string n = s.TrimToNull();
                if (n != null)
                ...
                baseRowFieldset.Count > 0)
            {
                ...
                model.RowBaseFields = new System.Collections.Generic.List<EntityField>();
            ...
            foreach (var field in fields)
            {
                ...
                if (foreign != null)
                {
                    ...
                    foreach (var frg in frgfld)
                    {
                        ...
                        var atk = new System.Collections.Generic.List<string>
                        {
                            "DisplayName(\"" + k.Title + "\")"
                        };
